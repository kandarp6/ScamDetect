...
async function runLiveScraper() {
  ...
  // Show status indicator and hide empty/results containers
  document.getElementById("scrape-status").style.display = "block";
  document.getElementById("scrape-empty").style.display = "none";
  document.getElementById("scrape-results-card").style.display = "none";
  
  var btn = document.getElementById("btn-start-scrape");
  btn.disabled = true;
  btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Scraping...';

  try {
    var response = await window.api.scrapeJobs(platform, keywords, limit);
    renderScrapedResults(response.results);
    
    // Refresh the statistics and recent lists
    await loadHomeStats();
    await loadRecentAnalyses();
  } catch (err) {
    alert("Scraping failed: " + err.message);
    document.getElementById("scrape-empty").style.display = "block";
  } finally {
    document.getElementById("scrape-status").style.display = "none";
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-play"></i> Start Live Scraper';
  }
}

function renderScrapedResults(results) {
  var tbody = document.getElementById("scrape-results-body");
  tbody.innerHTML = "";

  if (!results || results.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--ink-3)">No jobs found or all were duplicates.</td></tr>';
  } else {
    results.forEach(function(job) {
      var score = Math.round(job.scam_score || 0);
      var verdict = job.scam_risk_level || "Unknown";
      var hi = score > 65;
      var mid = score > 40;
      var color = hi ? "var(--red)" : mid ? "var(--amber)" : "var(--green)";
      ...
      tr.innerHTML = 
        ...
        '<td><span class="badge b-blue">' + (job.platform_name || "Other") + '</span></td>' +
        '<td>' + (job.city || "India") + '</td>' +
        '<td><strong style="color:' + color + ';font-family:var(--mono)">' + score + '/100</strong></td>' +
        '<td><span class="badge ' + badgeCls + '">' + verdict + '</span></td>' +
        '<td><a class="btn" href="' + job.source_url + '" target="_blank" style="padding:4px 8px;font-size:11px">View Original</a></td>';
